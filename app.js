...
function renderEmployeeCards() {
    ...
    if (front.length > 0) {
        html +=
            `<div class="position-group"><h3 class="position-title" style="display:flex;align-items:center;gap:8px;margin-bottom:12px;color:var(--info);"><i class="fas fa-door-open"></i> ${currentLanguage==='ja'?'フロント/サービス':'前台/服务'} <span class="position-count" style="font-size:12px;background:var(--info-light);color:var(--info);padding:2px 8px;border-radius:12px;">${front.length}</span></h3><div class="position-cards">${front.map(e=>generateEmployeeCard(e)).join('')}</div></div>`;
    }
    if (kitchen.length > 0) {
        html +=
...
function generateEmployeeCard(employee) {
    ...
    const posDisplay = currentLanguage === 'ja' ? (employee.position === '厨房区' ? '厨房' : 'フロント') :
...
function updateScheduleEmployeeSelect() {
    ...
    employees.sort((a,b) => a.name.localeCompare(b.name)).forEach(emp => {
        const pos = currentLanguage==='ja' ? (emp.position==='厨房区'?'厨房':'フロント') :
            (emp.position==='厨房区'?'厨房':'前台');
...
function renderWeeklySchedule() {
    ...
    days.forEach((day, i) => {
        ...
        const month = date.getMonth()+1, dayNum = date.getDate();
        html += `<div class="week-header-cell"><div style="font-weight:700;color:var(--dark);font-size:0.7rem;">${dayNames[i]}</div><div style="font-size:0.65rem;color:var(--gray-500);margin-top:2px;">${month}/${dayNum}</div></div>`;
    });
    html += `</div>`;

    // Build rows per position
    ...
    function buildRow(emp) {
        ...
        const posDisplay = currentLanguage==='ja'?(emp.position==='厨房区'?'厨房':'フロント'):
            (emp.position==='厨房区'?'厨房':'前台');
    ...
    if (frontDesk.length > 0) {
        html += `<div class="week-position-header front-desk"><i class="fas fa-door-open"></i> ${currentLanguage==='ja'?'フロント / サービス':'前台 / 服务'} <span class="week-position-count">${frontDesk.length}</span></div>`;
...
function editDaySchedule(employeeId, date) {
    ...
    <div class="edit-schedule-form">
      <div class="form-group"><label>${currentLanguage==='ja'?'スタッフ':'员工'}</label><div class="employee-display"><div class="employee-avatar-small">${emp.name.charAt(0)}</div><div><div style="font-weight:700;color:var(--dark);">${emp.name}</div><div style="font-size:14px;color:var(--gray-500);">${currentLanguage==='ja'?(emp.position==='厨房区'?'厨房':'フロント'):(emp.position==='厨房区'?'厨房':'前台')}</div></div></div></div>
...
function copyScheduleAsText() {
    ...
    text += `${currentLanguage==='ja'?'職種:':'职位:'} ${currentLanguage==='ja'?(emp.position==='厨房区'?'厨房':'フロント'):(emp.position==='厨房区'?'厨房':'前台')}\n`;
    ...
    text += `\n📍 ${currentLanguage==='ja'?'勤務エリア:':'工作区域:'} ${emp.position==='厨房区'?(currentLanguage==='ja'?'厨房 👨‍🍳':'厨房 👨‍🍳'):(currentLanguage==='ja'?'フロント/サービス 💁':'前台/服务 💁')}\n`;
...
function printEmployeeSchedule() {
    ...
    <div style="color:#666;font-size:11px;">${currentLanguage==='ja'?'職種:':'职位:'} ${currentLanguage==='ja'?(emp.position==='厨房区'?'厨房':'フロント'):(emp.position==='厨房区'?'厨房':'前台')}</div>
...
function printAllSchedule() {
    ...
    printContent += renderGroup(currentLanguage==='ja'?'フロント / サービス':'前台 / 服务', front);
...
function showTodaySchedule() {
    ...
    let html = '';
    if (front.length > 0) {
        html += `<h4 style="margin-bottom:16px;color:#2563eb;font-weight:700;"><i class="fas fa-door-open"></i> ${currentLanguage==='ja'?'フロント/サービス':'前台/服务'}</h4>`;
...
function createTodayItem(schedule) {
    const pos = currentLanguage==='ja'?(schedule.employeePosition==='厨房区'?'厨房':'フロント'):
...
function showStats() {
    ...
    <div class="stat-card"><h4>${frontCount}</h4><p>${currentLanguage==='ja'?'フロント':'前台'}</p></div>
