            ...
            ${days.map((day, index) => {
                ...
                return `
                    <div class="week-header-cell ${isToday ? 'today' : ''}">
                        <div style="font-weight: 700; color: var(--dark); font-size: 0.7rem;">${dayNames[index]}${isToday ? ` <span class="today-badge">${currentLanguage === 'ja' ? '今日' : '今天'}</span>` : ''}</div>
                        <div style="font-size: 0.65rem; color: var(--gray-500); margin-top: 2px;">${month}/${dayNum}</div>
                        <div class="week-header-count" title="${currentLanguage === 'ja' ? 'フロント / 厨房' : '前台 / 厨房'}">
                            <span class="count-front"><i class="fas fa-door-open"></i>${headcount.front}</span>
                            <span class="count-kitchen"><i class="fas fa-utensils"></i>${headcount.kitchen}</span>
                        </div>
                    </div>
                `;
            }).join('')}
